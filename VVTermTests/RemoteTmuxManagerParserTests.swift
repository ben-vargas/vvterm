...
struct RemoteTmuxManagerParserTests {
    private let scope: TmuxSocketScope = .userDefault
    ...
    func parseWhitespaceFormatFromRealTmuxOutput() {
        ...
        let sessions = RemoteTmuxManager.shared.parseSessionListOutput(output, scope: scope, allowLegacy: false)
    ...
    func parseLiteralEscapedTabsFormat() {
        ...
        let sessions = RemoteTmuxManager.shared.parseSessionListOutput(output, scope: scope, allowLegacy: false)
        #expect(sessions.count == 2)
        #expect(sessions[0] == RemoteTmuxSession(name: "prod", scope: scope, attachedClients: 2, windowCount: 3))
        #expect(sessions[1] == RemoteTmuxSession(name: "dev", scope: scope, attachedClients: 0, windowCount: 1))
    }
    ...
    func parseTwoFieldFormatDefaultsWindowCountToOne() {
        ...
        let sessions = RemoteTmuxManager.shared.parseSessionListOutput(output, scope: scope, allowLegacy: false)
        #expect(sessions.count == 2)
        #expect(sessions[0] == RemoteTmuxSession(name: "qa", scope: scope, attachedClients: 1, windowCount: 1))
        #expect(sessions[1] == RemoteTmuxSession(name: "local", scope: scope, attachedClients: 0, windowCount: 1))
    }
    ...
    func parseLegacyListSessionsFormatWhenEnabled() {
        ...
        let sessions = RemoteTmuxManager.shared.parseSessionListOutput(output, scope: scope, allowLegacy: true)
        #expect(sessions.count == 2)
        #expect(sessions[0] == RemoteTmuxSession(name: "ops", scope: scope, attachedClients: 1, windowCount: 2))
        #expect(sessions[1] == RemoteTmuxSession(name: "api", scope: scope, attachedClients: 0, windowCount: 1))
    }
